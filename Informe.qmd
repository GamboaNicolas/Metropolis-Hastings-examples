...
```{r}
grilla <- expand.grid(x1 = seq(-2.5, 2.5, length.out = 100),
            x2 = seq(-1, 6, length.out = 100)) 
grilla$densidad <- as.matrix(d_obj_p(grilla)) 
grilla |> `colnames<-`(c("x", "y", "Densidad"))

grilla |> filter((x > 0 & x < 1) & (y > 0 & y < 1)) |> summarise(prob = sum(Densidad))
