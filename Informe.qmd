...
Si se quiere obtener una muestra de la función de densidad $f(x)$ ,de la cual no es sencillo obtener muestras, el algoritmo de Metropolis-Hastings plantea que proponiendo una función $q(x)$ conocida, de la cual es sencillo muestrear, y un punto inicial ($\theta$) es posible obtener muestras de la función $f(x)$. 

El método funciona de la siguiente forma:

- De la distribución $q(x)$ centrada en $\theta$ muestrear un nuevo punto ($\theta'$)

- Si la densidad de la función $f(x)$ es mayor en $\theta'$ que en $\theta$, entonces $\theta'$ formará parte de la muestra, en caso contrario será parte de la muestra con probabilidad $\frac{f(\theta')}{f(\theta)}\frac{q(\theta \ |\theta')}{q(\theta' \ |\theta)}$ o con probabilidad $1-\frac{f(\theta')}{f(\theta)}\frac{q(\theta \ |\theta')}{q(\theta' \ |\theta)}$ se repetirá $\theta$ en la muestra.

- Repetir el proceso, obteniendo nuevos puntos propuestos a partir de $q(x)$ centrada en el último valor muestreado.

En definitiva, la probabilidad de seleccionar $\theta'$ como nuevo valor de la muestra es $\alpha = min\left\{1;\frac{f(\theta')}{f(\theta)}\frac{q(\theta \ |\theta')}{q(\theta' \ |\theta)}\right\}$
...
ggplot(tibble(x,f)) +
  geom_line(aes(x=x,y=f),linewidth=1,col="#1982C4") +
  geom_ribbon(aes(x=x,ymin=0,ymax=f),alpha=0.6,fill="#1982C4") +
  xlab(expression(theta)) +
  theme(axis.title.y = element_blank()) +
  geom_line(data=tibble(x=seq(-4,2,0.01),y=dnorm(x,-1,0.8)),
            aes(x=x,y=y), col="#AAFAC8") +
  geom_ribbon(data=tibble(x=seq(-4,2,0.01),y=dnorm(x,-1,0.8)),
              aes(x=x,ymin=0,ymax=y), fill="#AAFAC8",alpha=0.6) +
  ...
  geom_label(aes(x=-2, y=0.3),
             ...
             fill="#AAFAC8", 
             size=5 ,fontface="bold") +
  geom_text(x=-1, y=-0.13, label=expression(theta)) +
  geom_text(x=0.4, y=-0.1, label=expression(theta^"|")) +
...
A continuación se presentan las funciones usadas para aplicar el método en los casos en los que $f(\mu)$ es unidimensional y bidimensional:
...
sample_mh <- function(n, d_objetivo, r_propuesta = NULL, d_propuesta = NULL,
                      p_inicial = NULL){
...
1. $\Sigma_1 = \begin{bmatrix} 1 & 0 \\ 0 & 2 \end{bmatrix}$ Se puede ver en @fig-cov1, @fig-trace-cov1 y @fig-autocor1

2. $\Sigma_2 = \begin{bmatrix} 0.25 & 0 \\ 0 & 0.25 \end{bmatrix}$ Se puede ver en @fig-cov2, @fig-trace-cov2 y @fig-autocor2

3. $\Sigma_3 = \begin{bmatrix} 1 & 0.3 \\ 0.3 & 1 \end{bmatrix}$ Se puede ver en @fig-cov3, @fig-trace-cov3 y @fig-autocor3
...
1. $\Sigma_1 = \begin{bmatrix} 0.06 & 0 \\ 0 & 0.07 \end{bmatrix}$ Se puede ver su densidad en @fig-banana1, el recorrido de la cadena en @fig-bananatrace1 y la autocorrelación en @fig-bananacor1. Con esta matriz la probabilidad de aceptación es `r round(cadena_p1$probabilidad_aceptacion, 4)`.
...
2. $\Sigma_2 = \begin{bmatrix} 1 & 0 \\ 0 & 1 \end{bmatrix}$ Se puede ver su densidad en @fig-banana2, el recorrido de la cadena en @fig-bananatrace2 y la autocorrelación en @fig-bananacor2. Con esta matriz la probabilidad de aceptación es `r round(cadena_p2$probabilidad_aceptacion, 4)`.
...
3. $\Sigma_3 = \begin{bmatrix} 0.5 & 0 \\ 0 & 0.5 \end{bmatrix}$ Se puede ver su densidad en @fig-banana3, el recorrido de la cadena en @fig-bananatrace3 y la autocorrelación en @fig-bananacor3. Con esta matriz la probabilidad de aceptación es `r round(cadena_p3$probabilidad_aceptacion, 4)`.
...
Considerando en los tamaños de las muestras efectivas en cada dimensión y en los gráficos de autocorrelación observados, parece que la matriz de covarianzas $\Sigma_2$ genera muestras que ajustan mejor a la distribución objetivo.
