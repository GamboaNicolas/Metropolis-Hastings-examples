...
rhat <- R_hat(cbind(df_cadenas$data_cadena_1$cadena[2],
                    df_cadenas$data_cadena_2$cadena[2],
                    df_cadenas$data_cadena_3$cadena[2]))
```
...
Teniendo en cuenta que estas muestras no son independientes resulta de interés conocer a cuantas muestras independientes equivalen, para ello se calcula el número efectivo de muestras $(n_{eff})$ para cada cadena.

Para el $\kappa = 1$, el $n_{eff} =$ `r floor(neff1)`, para el $\kappa = 2$, el $n_{eff} =$ `r floor(neff2)` y para el $\kappa = 5$, el $n_{eff} =$ `r floor(neff3)`, por lo tanto las muestras obtenidas a partir de un muestreo con un $\kappa = 5$ son las que equivalen a un mayor número de muestras independientes. (Redactar mejor)
