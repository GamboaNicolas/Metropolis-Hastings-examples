...
Una vez ejecutado el algoritmo terminaremos con una muestra de tamaño n: $\{\theta^{(1)},\theta^{(2)},...,\theta^{(n)}\}$

Para obtener cada muestra $\theta^{(i+1)}$ se partirá desde $\theta^{(i)}$
    1. se propondrá un nuevo valor $\theta'$ en función de $q(\theta' \ |\theta^{(i)})$
    2. Se calcula la probabilidad de que $\theta^{(i+1)}$ tome el valor de $\theta'$: $\alpha = min\left\{1;\frac{f(\theta')}{f(\theta)}\frac{q(\theta \ |\theta')}{q(\theta' \ |\theta)}\right\}$
    3. 
...
1. $\Sigma_1 = \begin{bmatrix} 0.06 & 0 \\ 0 & 0.07 \end{bmatrix}$ Se puede ver su densidad en @fig-banana1, el recorrido de la cadena en @fig-bananatrace1 y la autocorrelación en fig-banana-autocor1. Con esta matriz la probabilidad de aceptación es `r round(cadena_p1$probabilidad_aceptacion, 4)`.
...
2. $\Sigma_2 = \begin{bmatrix} 1 & 0 \\ 0 & 1 \end{bmatrix}$ Se puede ver su densidad en @fig-banana2, el recorrido de la cadena en @fig-bananatrace2 y la autocorrelación en fig-banana-autocor2. Con esta matriz la probabilidad de aceptación es `r round(cadena_p2$probabilidad_aceptacion, 4)`.
...
3. $\Sigma_3 = \begin{bmatrix} 0.5 & 0 \\ 0 & 0.5 \end{bmatrix}$ Se puede ver su densidad en @fig-banana3, el recorrido de la cadena en @fig-bananatrace3 y la autocorrelación en fig-banana-autocor3. Con esta matriz la probabilidad de aceptación es `r round(cadena_p3$probabilidad_aceptacion, 4)`.
...
filas <- c("M-H", "Integración \\ numérica")
